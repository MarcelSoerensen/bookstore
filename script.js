...
function renderBookInfo() {
  let bookInfoRef = document.getElementById('book_info_boxes');
  bookInfoRef.innerHTML = '';
    
  for (let bookIndex = 0; bookIndex < books.length; bookIndex++) {
    
    let bookCommentsRef = '';
    for (let commentIndex = 0; commentIndex < books[bookIndex].comments.length; commentIndex++) {
      bookCommentsRef += /*html*/`
          <tr>
...
function toggleHeartAndCounter(bookIndex) {
  let toggleHeartRef = document.getElementById(`like_heart${bookIndex}`);
  let likesCounterRef = document.getElementById(`likes_counter${bookIndex}`)
  
  if (books[bookIndex].liked) {
    toggleHeartRef.classList.remove('red_heart');
    books[bookIndex].likes -= 1;
    
    } else {
    toggleHeartRef.classList.add('red_heart');
    books[bookIndex].likes += 1;
    }
